        ...
        <div className="flex border-b border-slate-100">
          {TABS.map(([k, l]) => (
            <button key={k} type="button" onClick={() => setSection(k)}
              className={`flex-1 py-2.5 text-xs font-semibold transition-colors ${section === k ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>{l}</button>
          ))}
        </div>
      ...
      {open && (
        <tr>
          <td colSpan={9} className="px-4 pb-3 bg-slate-50">
            ...
            <div className="pt-2">
              <button onClick={e=>{e.stopPropagation();onRefer(patient);}} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 border border-blue-200 text-blue-700 text-xs font-semibold hover:bg-blue-100 transition-colors">
...
export default function PatientRegistry() {
  ...
  return (
    <div className="space-y-5">
      ...
      {referralPatient && (
        <ReferralModal patient={referralPatient} onClose={() => setReferralPatient(null)} currentFacilityName="Kakumiro HC III" />
      )}

      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Users className="h-6 w-6 text-blue-600" />Patient Registry</h1>
          <p className="text-sm text-slate-500 mt-0.5">Kakumiro HC III — {patients.length} registered patients</p>
        </div>
