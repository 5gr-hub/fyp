...
function Sidebar({ mobile, onClose, drcpUser, onDrcpLogin, onDrcpLogout }) {
  return (
    <div className={`flex flex-col h-full bg-slate-900 text-white ${mobile ? 'w-72' : 'w-60'}`}>
      <div className="px-5 py-5 border-b border-slate-700/60 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            <Activity className="h-5 w-5 text-blue-400" />
            <span className="font-bold text-sm text-white tracking-tight">Kakumiro HC III</span>
          </div>
      ...
      <div className="px-4 py-4 border-t border-slate-700/60 space-y-2">
        ...
        <p className="text-[10px] text-slate-600">API: <span className="text-slate-500 font-mono">localhost:4001</span></p>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  // Sync DRCP auth state when sessionStorage changes (e.g., login inside ReferralModal)
  useEffect(() => {
  ...
  const handleDrcpLogout = () => {
    sessionStorage.removeItem(SESSION_KEY);
    sessionStorage.removeItem(USER_KEY);
    setDrcpUser(null);
  };
  const handleDrcpAuthSuccess = () => {
    setDrcpUser(getUser());
    setShowDrcpLogin(false);
  };
  ...
  return (
    <BrowserRouter>
      <div className="flex h-screen overflow-hidden">
        {/* DRCP login trigger from sidebar — opens a bare ReferralModal with no patient to force login */}
        {showDrcpLogin && (
          <ReferralModal
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <header className="bg-white border-b border-slate-200 px-4 py-3 flex items-center gap-3 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 border border-blue-200 text-blue-700 rounded-full text-xs font-semibold">
                <span className="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse" />
                Kakumiro HC III — HMIS Live
              </span>
            </div>
            {drcpUser && (
