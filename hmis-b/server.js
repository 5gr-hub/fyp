...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', system: 'St. Francis Nsambya Hospital HMIS', version: '2.0' });
});
...
app.post('/api/patients', (req, res) => {
  ...
  const patient = {
    id: `MUB-${String(nextNum).padStart(3, '0')}`,
    full_name: req.body.full_name ?? '',
    sex: req.body.sex ?? 'unknown',
    date_of_birth: req.body.date_of_birth ?? '',
    identifier: req.body.identifier ?? `CM${Date.now()}MUB`,
...
app.post('/api/queue', (req, res) => {
  ...
  const entry = {
    ...
    department: req.body.department ?? 'OPD',
    status: 'waiting',
...
app.post('/api/triage', (req, res) => {
  ...
  if (qi !== -1) {
    db.queue[qi].status = 'triaged';
    db.queue[qi].triage_id = entry.id;
    db.queue[qi].priority = req.body.triage_category === 'emergency' ? 'emergency' : req.body.triage_category === 'urgent' ? 'urgent' : 'normal';
  }
  saveData(db);
  res.status(201).json(entry);
});
...
app.post('/api/encounters', (req, res) => {
  ...
  const entry = {
    ...
    plan: req.body.plan ?? {},
    clinician: req.body.clinician ?? 'Dr. Namukasa',
...
app.listen(PORT, () => console.log(`St. Francis Nsambya Hospital HMIS API v2 on http://localhost:${PORT}`));
