...
function DispenseModal({ inventory, patients, onClose, onSaved }) {
  const [form, setForm] = useState({ patient_id:'', patient_name:'', medication_id:'', quantity:1, dispensed_by:'Pharm. Nantume', dosage_instructions:'', notes:'' });
...
function AddItemModal({ onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="font-bold text-slate-800 flex items-center gap-2"><PackagePlus className="h-4 w-4 text-green-600"/>Add New Item to Inventory</h2>
          <button onClick={onClose}><X className="h-5 w-5 text-slate-400"/></button>
        </div>
        <form onSubmit={save} className="p-6 space-y-3">
          {err && <div className="p-3 rounded-lg bg-red-50 border border-red-200 text-red-700 text-sm">{err}</div>}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="font-bold text-slate-800 flex items-center gap-2"><RefreshCw className="h-4 w-4 text-green-600"/>Restock</h2>
          <button onClick={onClose}><X className="h-5 w-5 text-slate-400"/></button>
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Pill className="h-6 w-6 text-green-600"/>Pharmacy &amp; Inventory</h1>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredInv.map(m=>(
                <tr key={m.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={()=>setModal(m)} className="text-xs font-semibold text-green-600 hover:text-blue-800 flex items-center gap-1">
