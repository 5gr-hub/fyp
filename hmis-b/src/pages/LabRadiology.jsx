...
function RequestModal({ patients, onClose, onSaved }) {
  ...
  const [requestedBy, setRequestedBy] = useState('Dr. Namukasa');
            ...
            <div className="border border-slate-200 rounded-lg max-h-48 overflow-y-auto divide-y divide-slate-100">
              {filtered.map(t => (
                <button key={t} type="button" onClick={() => { setTestType(t); setSearch(''); }}
                  className={`w-full text-left px-3 py-2 text-sm transition-colors ${testType === t ? 'bg-green-50 text-green-700 font-semibold' : 'hover:bg-slate-50 text-slate-700'}`}>
                  <span className={`inline-block w-1.5 h-1.5 rounded-full mr-2 ${LAB_PANELS[t].category === 'radiology' ? 'bg-purple-400' : 'bg-green-400'}`} />
                  {t}
                </button>
              ))}
            </div>
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <FlaskConical className="h-6 w-6 text-green-600" /> Lab &amp; Radiology
          </h1>
          ...
          <div key={r.id} className="px-5 py-4 flex items-start gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                <span className={`badge border capitalize ${r.category==='radiology'?'bg-purple-50 text-purple-700 border-purple-200':'bg-green-50 text-green-700 border-green-200'}`}>{r.category}</span>
