...
import { Users, ClipboardList, FlaskConical, Pill, AlertTriangle, Activity, Clock } from 'lucide-react';

const API = '/api';

function StatCard({ icon: Icon, label, value, sub, color = 'green', alert = false }) {
  const colors = {
...
function QueueItem({ entry }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-slate-100 last:border-0">
      <div className={`h-2 w-2 rounded-full flex-shrink-0 ${entry.priority === 'emergency' ? 'bg-red-500 animate-pulse' : entry.priority === 'urgent' ? 'bg-amber-400' : 'bg-green-400'}`} />
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
          <Activity className="h-6 w-6 text-green-600" /> Dashboard
        </h1>
        <p className="text-sm text-slate-500 mt-0.5">
          St. Francis Nsambya Hospital — {new Date().toLocaleDateString('en-UG', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
          ...
          <p className="text-2xl font-bold text-green-600">{activeQueue.filter(q => q.status === 'triaged').length}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
