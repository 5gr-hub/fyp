...
function TriageModal({ entry, onClose, onSaved }) {
  ...
  const [triagedBy, setTriagedBy] = useState('Nurse Nantume');
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 overflow-y-auto flex items-start justify-center p-4 pt-8">
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 className="font-bold text-slate-800 flex items-center gap-2"><Activity className="h-4 w-4 text-green-600"/>Triage — {entry.patient_name}</h2>
...
function ConsultModal({ entry, triageData, onClose, onSaved }) {
  ...
  const [clinician, setClinician] = useState('Dr. Namukasa');
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 overflow-y-auto flex items-start justify-center p-4 pt-8">
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            <h2 className="font-bold text-slate-800 flex items-center gap-2"><Stethoscope className="h-4 w-4 text-green-600"/>Clinical Note — {entry.patient_name}</h2>
        ...
        <div className="flex border-b border-slate-100">
          {TABS.map(([k,l])=>(
            <button key={k} onClick={()=>setTab(k)} className={`flex-1 py-3 text-sm font-semibold transition-colors ${tab===k?'text-green-600 border-b-2 border-green-600':'text-slate-400 hover:text-slate-600'}`}>
    ...
    <div className="fixed inset-0 bg-black/60 z-50 overflow-y-auto flex items-start justify-center p-4 pt-8">
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl">
        ...
        <div className="p-6 space-y-5">
          {enc && (
            <div className="rounded-xl bg-green-50 border border-blue-100 p-4 space-y-1">
              <p className="text-xs font-semibold text-green-700 uppercase tracking-wide">Diagnosis</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><ClipboardList className="h-6 w-6 text-green-600"/>Consultation Queue</h1>
          <p className="text-sm text-slate-500 mt-0.5">St. Francis Nsambya Hospital — clinical workflow</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[['waiting','Waiting',Clock,'text-yellow-600 bg-yellow-50 border-yellow-200'],['triaged','Triaged',Activity,'text-green-600 bg-green-50 border-green-200'],['in_consultation','Consulting',Stethoscope,'text-purple-600 bg-purple-50 border-purple-200'],['discharged','Discharged',CheckCircle2,'text-emerald-600 bg-emerald-50 border-emerald-200']].map(([k,l,Icon,c])=>(
            ...
            <div key={entry.id} className={`px-5 py-4 flex items-center gap-4 border-l-4 ${STATUS_COLOR[entry.status]??'border-l-slate-200'}`}>
              ...
              <div className="flex flex-col gap-1.5 flex-shrink-0">
                ...
                {(entry.status==='in_consultation'||entry.status==='orders'||entry.status==='discharged') && (
                  <button onClick={()=>openReferral(entry)} className="flex items-center gap-1 text-[11px] font-semibold text-green-600 hover:text-blue-800 transition-colors">
